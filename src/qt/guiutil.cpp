...
{
    // Convert CEED:// to CEED:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("CEED://"))
    {
        uri.replace(0, 12, "CEED:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CEED.lnk";
}
...
{
    return GetAutostartDir() / "CEED.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CEED-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  CEED-qt [" + tr("command-line options") + "]                     " + "\n";
